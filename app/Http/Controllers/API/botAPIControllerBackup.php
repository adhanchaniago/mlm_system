...
class botAPIController extends AppBaseController
{
	...
	public function instance_campaign(Request $request){
		...
		$company_id = $user['typeid'];
		if(bot::where('instance_id',$instance_id)->where('company_id',$company_id)->exists()) {
			$bots = bot::where('instance_id',$instance_id)->where('company_id',$company_id)->first();
			if(!empty($bots->botCampaign)){
				if($bots->botCampaign->exists()){
//		            $bot_campaigns   = $bots->botCampaign;
					$bot_campaigns   = botCampaign::where('bot_id',$bots->bot_id)->get();
					foreach ($bot_campaigns as $bot_campaign){
//                      $campaigns       = $bot_campaign->campaigns;
						$campaigns       = campaigns::where('campaign_id',$bot_campaign->campaign_id)->first();
						$campaign_view    = DB::table('campaign_view')->where('user_id', $user_id)->where('campaign_id', $campaigns->campaign_id)->first();
						if (empty($campaign_view)) {
							DB::table('campaign_view')->insert([
								['user_id' => $user_id, 'campaign_id' => $campaigns->campaign_id, 'company_id' => $campaigns->company_id, 'date' => date('d-m-Y')]
							]);
						}
						...
						if(favorite::where('user_id',$user_id)->where('campaign_id',$campaigns->campaign_id)->exists()){
						...
						if($campaign_category1 = categoryCampaign::where('campaign_id',$campaigns->campaign_id)->exists()){
							$campaign_category1 = categoryCampaign::where('campaign_id',$campaigns->campaign_id)->first();
							$cat= category::where('id',$campaign_category1->category_id)->first();
							$campaigns['campaign_category']=$cat->category_name;
							$campaigns['category_id']=$campaign_category1->category_id;
						}else{
...
//		            #################################
					if(!empty($user_id)){
						$company_user    = DB::table('company_user_list')->where('user_id', $user_id)->where('company_id', $company_id)->first();
						if (empty($company_user)) {
							DB::table('company_user_list')->insert([
								['user_id' => $user_id, 'company_id' => $company_id, 'date' => date('d-m-Y')]
							]);
							// **************************************************************************************************
							//here it goes to mailchimp
							$list = DB::table('company_mailchimp_list')->where('company_id',$company_id)->where('Is_Prospect',1)->first();
							if(!empty($list) && !empty($user)){
								...
								DB::table('company_user_list')->where('user_id',$user_id)->where('company_id',$company_id)->update($input);
								curl_close($ch);
								// **************************************************************************************************
							}
						}
