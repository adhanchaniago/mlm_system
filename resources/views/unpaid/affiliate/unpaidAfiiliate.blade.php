...
@section('content')
    <section class="content-header">
        <h1 class="pull-left">Unpaid SamyAffiliate Companies</h1>
        <h1 class="pull-right">
            ...
            <div class="box-body scrollable">
                <table class="table table-responsive" id="unpaid-table">
                    ...
                    @foreach($users as $user)
                        ...
                        $plan = \App\Models\plantable::whereId($company->planid)->first();
                        ?>
                        <tr>
                            <td>{{$i}}</td>
                            <td>{{$company->name}}</td>
                            ...
                            <td>
                                <div class='btn-group'>
                                    <center>
                                        <a href="{{ url('samy_affiliate/show'.'/'.$company->id) }}" class='btn btn-default btn-xs'>
                                            <i class="glyphicon glyphicon-eye-open"></i>
                                        </a>
                                        @if($user->affiliate_disabled == 0)
                                            <button type="button" class="btn btn-danger btn-xs" onclick="disableCompany('{{$company->id}}',1,'affiliate_disabled')" title="Suspend User"><i class="fa fa-ban"></i></button>
                                        @else
                                            <button type="button" class="btn btn-success btn-xs" onclick="disableCompany('{{$company->id}}',0,'affiliate_disabled')" title="Enable User"><i class="fa fa-check"></i></button>
                                        @endif
                                    </center>
                                </div>
                            </td>
