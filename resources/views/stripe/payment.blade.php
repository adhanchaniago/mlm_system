        ...
        <div class="col-md-12 stripe-pay_section">
            ...
            <div class="panel">
                <div class="panel-body">
                    <form class="form-horizontal" method="POST" id="payment-form" role="form" action="{!! URL('samybot/ReSubscription') !!}">
                        {{ csrf_field() }}

                        <input type="hidden" class="form-control" name="stripe_plan_id" value="{{ $plan->stripe_paln_id }}">
                            ...
                            <div class="modal-dialog add-level-modal">
                                <div class="modal-content add-level-content">
                                    ...
                                    <div class="modal-body stripe-modal-body">
                                        <?php
                                            $planDetails = App\Models\SamyBotPlans::whereId($plan->plan)->first();
                                        ?>
                                        <table class="col-md-12 col-xs-12 summary table-bordered" style="text-align: right">
                                            <input type="hidden" class="form-control" name="term" value="{{ $planDetails->term }}">
                                            <tbody style="border: 1px solid">
                                            <tr>
                                                <td class="text-left">{{trans('samybot.common.samyBot')}}</td>
                                                <td class="text-center">{{$planDetails->name}}</td>
                                            </tr>
                                            <tr>
                                                <td class="text-left">{{trans('samybot.checkout.price_per_device')}}</td>
                                                <td class="text-center">{{$amount}}</td>
                                            </tr>
                                            <tr>
                                                <td class="text-left">{{trans('samybot.common.device')}}</td>
                                                <td class="text-center">{{$plan->quantity}}</td>
                                            </tr>
                                            <tr>
                                                <td class="text-left">{{trans('samybot.checkout.renewal_charge')}}</td>
                                                <td class="text-center">{{$total}}</td>
                                            </tr>
                                            </tbody>
                                        </table>
                                        <div class="col-md-12">
                                            <button type="submit" id="paywithStripe" class="btn-save-level btn-row">{{trans('home.proceed_to_pay')}}</button>
                                        </div>
                                    </div>
