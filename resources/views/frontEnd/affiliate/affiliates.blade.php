    ...
    <!--SideBar end-->
        ...
        @if($affiliates != "")
            <div class="col-md-10 col-xs-12 col-sm-10 section2_numbers">
                <div class="col-md-10 col-xs-12 col-sm-12 table-responsive">
                    <table class="col-md-12">
                        <tbody>
                        @foreach($affiliates as $affiliate)
                            <?php
                            $affiliate_user = \App\User::where('affiliate_id', $affiliate->id)->first();
                            if ($affiliate_user->status == '4')
                            {
                                if (\Illuminate\Support\Facades\DB::table('bot_plans')->where('company_id',$affiliate_user->company_id)->where('payment_status',1)->exists()==0)
                                {
//                                    continue;
                                }
                            }
